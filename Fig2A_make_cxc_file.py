...
def main():
	...
	for curr_index, curr_row in mageck_df.iterrows():
		...
		for aa_num in aas_to_colors:
			...
			if aa_num in colored_dict:
				if np.abs(colored_dict[aa_num]) > np.abs(curr_LFC):
					continue

			colored_dict[aa_num] = curr_LFC
	...
	out_fp.write('view matrix camera 0.8624,-0.46189,-0.20718,-58.462,-0.29172,-0.11896,-0.94908,-350,0.41373,0.87892,-0.23733,-82.939\n')
	out_fp.write('view matrix camera -0.67165,0.019634,-0.74061,-270.12,-0.71619,0.23866,0.65583,256.83,0.18964,0.9709,-0.14623,-62.232\n')

	# Outline structure for zoom
	# out_fp.write('graphics silhouettes true width 2 color dim grey\n')
	# out_fp.write('surface\n')
	# out_fp.write('transparency 70 surfaces\n')
